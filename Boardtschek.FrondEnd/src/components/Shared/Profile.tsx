...
import axios from "axios"; // Import the interface
        ...
        <div className="min-h-screen bg-gray-100 p-8">
            <div className="max-w-4xl mx-auto bg-white shadow rounded-lg p-6">
                ...
                <div className="mb-6">
                    ...
                    >
                        {isPasswordChangeVisible ? "Cancel Password Change" : "Change Password"}
                    </button>
                    {isPasswordChangeVisible && (
                        <form onSubmit={handleSubmit(handlePasswordSubmit)} className="mt-4 space-y-4">
                            <div>
                    ...
                    <ul className="list-disc pl-6">
                        {profileData.rentals && profileData.rentals.length > 0 ? (
                            profileData.rentals.map((rental, index) => (
                                <li key={index}>
                                    {rental.title || "Unknown Game"} - Rented on {rental.rentalDate ? new Date(rental.rentalDate).toLocaleDateString() : "Unknown Date"}
                                </li>
                            ))
                        ) : (
                            <p>No rentals yet.</p>
